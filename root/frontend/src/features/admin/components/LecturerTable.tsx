...
import { deleteLecturerByIdAPI, getAllLecturersAPI } from "../api/lecturers";
...
export default function LecturerTable() {
  ...
  const fetchLecturers = useCallback(
    async (token: string, page: number = 1) => {
      const response: Response | undefined = await getAllLecturersAPI(
        token,
        pageSize,
        page,
        searchTerm
  ...
  useEffect(() => {
    if (!authToken) return;
    fetchLecturers(authToken, currentPage);
  }, [authToken, currentPage, fetchLecturers]);
  ...
  const handleDelete = async (lecturerId: number) => {
    ...
    const response = await deleteLecturerByIdAPI(authToken, lecturerId);
    if (response && response.ok) {
      navigate("/admin/lecturers");
      fetchLecturers(authToken, currentPage);
      toast.success(`Deleted lecturer ${lecturerId}`);
      return;
    } else {
      toast.error(`Failed to delete lecturer ${lecturerId}`);
      return;
    }
  };
            ...
            <table className="min-w-full text-left">
              <thead className="bg-slate-50 text-slate-500">
                <tr className="text-sm">
                  <th className="px-6 py-4 font-medium">Lecturer ID</th>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                ...
                {lecturers.map((lecturer) => (
                  <tr key={`${lecturer.lecturerId}`} className="text-sm">
                    <td className="px-6 py-5">{lecturer.lecturerId}</td>
                    ...
                    <td className="px-6 py-5">{lecturer.email}</td>
                    <td className="px-6 py-5">{lecturer.phoneNumber}</td>
                    <td className="px-6 py-5 text-slate-500">
                      <Link
                        to={`/admin/lecturers/${lecturer.lecturerId}/edit`}
